...
class DataBase():
    def __init__(self):
        ...
        try:
            self.cursor.execute(
                "CREATE TABLE patient(id INTEGER PRIMARY KEY AUTOINCREMENT, nom TEXT, prenom TEXT, contact TEXT, adresse TEXT, referent TEXT, date TEXT)")
    ...
    def setPatient(self, values):
        self.cursor.execute(
            "INSERT INTO patient(nom, prenom, contact, adresse, referent, date) VALUES(?,?,?,?,?,?)", values)
        self.connector.commit()
