...
class MyWindow:
    ...
    def __init__(self):
        ...
        label_age = Label(self.frame_home, text="age", font=FONT,
                          relief="flat", bg="powderblue", fg="black")
        self.entry_age = Entry(self.frame_home,  font=FONT,
                               relief="flat", bd=5, bg="#eee")

        label_contact = Label(self.frame_home, text="contact",
                              font=FONT, relief="flat", bg="powderblue", fg="black")
        self.entry_contact = Entry(
            self.frame_home,  font=FONT, relief="flat", bd=5, bg="#eee")

        # address
        label_address = Label(self.frame_home, text="address",
                              font=FONT, relief="flat", bg="powderblue", fg="black")
        ...
        # envoyer
        envoyer_btn = Button(self.frame_home, text="Envoyer",  font=FONT,
                             relief="flat", bd=5, bg="#eee", command=self.insert_db)

        # label_compte
        label_compte = Label(
            self.frame_account, text="Déjà un compte?  Entrez votre id", fg="#000", font=('', 14))

        self.entry_cree = Entry(self.frame_account, text="compte",
                                font=FONT, relief="flat", bg="#eee", fg="black")
    ...
    def insert_db(self):
        ...
        values = (self.entry_nom.get().strip(), self.entry_prenom.get().strip(), self.entry_age.get().strip(
        ), self.entry_contact.get().strip(), self.entry_address.get().strip(), self.entry_reference.get().strip(), date)
        self.liste_entr = [self.entry_nom, self.entry_prenom, self.entry_age,
    ...
    def services_desc(self, cmpt):
        ...
        serv = Label(self.frame_service, text="service",
                     bg="powderblue", font=FONT, width=20)
        ...
        batiment = Label(self.frame_service, text="Batiment",
                         bg="powderblue", font=FONT)
    ...
    def menu(self):
        ...
        menu_lister.add_command(label="Patients", command=Lister)
        principale.add_cascade(label="Lister", menu=menu_lister)
